...
__global__ void kernel_near_far_from_aabb(
    ...
    // 获取到原点和方向的各值ox,oy,oz,dx,dy,dz
    const float ox = rays_o[0], oy = rays_o[1], oz = rays_o[2];
    const float dx = rays_d[0], dy = rays_d[1], dz = rays_d[2];
    const float rdx = 1 / dx, rdy = 1 / dy, rdz = 1 / dz;

    // get near far (assume cube scene)
    // 按照x轴计算出near和far，即(x_min - ox) / dx和(x_max - ox) / dx
    // 其中x_min和x_max就是-bound和bound
    ...
    // 如果near_x大于far_y或者near_y大于far_x，将near和far直接设置为infinity
    if (near > far_y || near_y > far) {
        nears[n] = fars[n] = std::numeric_limits<scalar_t>::max();
        return;
    }
    // near = min{near_x, near_y}   far = max{far_x, far_y}
    ...
    // 如果min{near_x, near_y}大于far_z或者near_z大于max{far_x, far_y}，将near和far直接设置为infinity
    if (near > far_z || near_z > far) {
        nears[n] = fars[n] = std::numeric_limits<scalar_t>::max();
        return;
    }
    // near = min{near_x, near_y, near_z}   far = max{far_x, far_y, far_z}
...
__global__ void kernel_sph_from_ray(
    ...
    // 计算射线与半径为radius的球体的交点，结果是t，交点坐标就是o+td
